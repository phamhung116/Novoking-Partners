...
const CancelIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
const CompleteIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
...
const SystemNotificationCard: React.FC<SystemNotificationCardProps> = ({ notification, onClick }) => {
    const Icon = ICONS[notification.icon].component;
    const iconColor = ICONS[notification.icon].color;

    return (
        <div 
            className="relative flex-shrink-0 w-40 h-48 bg-white rounded-lg border border-gray-200 p-4 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 hover:shadow-lg hover:scale-105 transition-all duration-300"
            onClick={() => onClick(notification)}
        >
            {!notification.isRead && (
                <div className="absolute top-2 right-2 h-2.5 w-2.5 bg-primary rounded-full"></div>
            )}
            <div className={`h-16 w-16 rounded-full flex items-center justify-center mb-3 ${iconColor}`}>
                <Icon />
            </div>
            <p className="text-sm font-semibold text-gray-800 leading-tight line-clamp-3 flex-grow">{notification.title}</p>
            <p className="text-xs text-gray-400 mt-auto pt-2">{notification.timestamp}</p>
